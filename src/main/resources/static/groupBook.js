(function() {
	...
	const addEventListeners = function() {
		const addReaderProgressRecordButton = document
				.querySelector('#addReaderProgressRecordButton');
		addReaderProgressRecordButton.addEventListener('click',
				addReaderProgressRecord);
	};

	const addReaderProgressRecord = function() {
		var addReaderProgressRecordForm = document
				.getElementById('addReaderProgressRecordForm');
		...
		xhr.onreadystatechange = function() {
			if (this.status === 200 && this.readyState === 4) {
				if (this.responseText) {
					...
					const newReaderProgressRecord = JSON
